...
struct
  ...
  val focusEntry = focus entryUid
  val focusExit = fn graph =>
    let
  ...
  local
    fun prepareForSplicing (graph: graph) (single: tail -> 'a)
      ...
      let
        ...
        val ((_, entryTail), graph) = focusEntry graph
      in
        if IntRedBlackMap.isEmpty graph then
          ...
          let
            ...
            val (exitBlock, graph) = focusExit graph
  ...
  fun spliceHeadOnly (head: head) (graph: graph) : graph =
    let
      val (gentry, graph) = focusEntry graph
    in
      case gentry of
  ...
  fun reversePostorderDfs (graph: graph) : block list =
    let
      val (entry, blocks) = focusEntry graph
  ...
  fun exit ((head, tail), graph) =
    (unreachable tail; ((head, Last Exit), graph))
